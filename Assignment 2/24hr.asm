...
CHECKHOUR:						;check if hour <= 24
	MOV A,H
	CPI 24H
	JC CHECKMIN
SETHOUR:						;set hours to 0 if hours>=24
	MVI H,00H
	;JC SKIPCHECK
...
UPDATE_HOUR_MIN:				
	...
	MVI B,00H
	CALL ALARM
	MVI A,00H     				;start seconds counter from 0s
	ADD B 						; Add 7 seconds delay to accumulator incase of alarm
	MVI B,00H
NEXTSEC:
	STA CURDT
...
ALARM: 						;CHECK IF CURRENT TIME == ALARM
	LDA 8844H				;COMPARE HOUR
	CMP H
	JNZ RETURN
	LDA 8845H 				;COMPARE MINUTE
	CMP L
	JNZ RETURN
	
    MVI A,0H
	LXI H,8840H				;DISPLAY 'AS02' in address field for 7 seconds
	CALL OUTPUT
	...
	CALL CLEAR 
	MVI B,07H 				; add 7 seconds to accumulator upon return
RETURN:
	RET