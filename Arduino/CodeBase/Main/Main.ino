...
//start position initializer ping
int trigPin = 33;//33
int echoPin = 32;//32
...
void setup() {
  ...
  startPos = 1; //hardcode start pos. To be solved later (using rotary encoder)
...
void setStart()
{
  ...
  Serial.println(dist);
  delay(500);
  while(dist>18) //final calibration
  {
...
void lockOpen()
{
  Serial.println("lockOpen");
  for(int i = 0;i< 5;i++)
  {
    Serial.println("lock open ho ja 55");
  }
  lock.write(120);
}
...
void lockClose()
{
  Serial.println("lockClose");
  
  for(int i = 0;i< 5;i++)
  {
    Serial.println("lock close ho ja 55");
  }
  lock.write(30);
}
...
bool checkhand()
{
  ...
  delay(1000);
  dist3 = calculateDistance();
  delay(1000);
  while(dist1<15 || dist2<15 || dist3<15)
  {
    dist1=dist2;
    dist2=dist3;
    dist3 = calculateDistance();
